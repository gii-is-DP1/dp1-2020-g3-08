...
public class UsuarioRegistradoController {
	...
	private static final String				VIEWS_USUARIOREGISTRADO_SHOW					= "usuariosRegistrados/usuarioRegistradoDetails";
	...
	public Collection<Sexo> populateGenres() {
		return usuarioRegistradoService.findGenres();
	}
	...
	public String processCreationForm(@Valid UsuarioRegistrado usuarioRegistrado, BindingResult result) {
		if (result.hasErrors()) {
			...
			usuarioRegistradoService.saveUsuarioRegistrado(usuarioRegistrado);
	...
	public String initUpdateUsuarioRegistradoForm(@PathVariable("usuarioRegistradoID") int usuarioRegistradoID, Model model) {
		UsuarioRegistrado usuarioRegistrado = usuarioRegistradoService.findUsuarioRegistradoById(usuarioRegistradoID);
		model.addAttribute(usuarioRegistrado);
		return VIEWS_USUARIOREGISTRADO_CREATE_OR_UPDATE_FORM;
	}

	@PostMapping(value = "/usuariosRegistrados/{usuarioRegistradoID}/edit")
	public String processUpdateUsuarioRegistradoForm(@Valid UsuarioRegistrado usuarioRegistrado, BindingResult result, @PathVariable("usuarioRegistradoID") int usuarioRegistradoID) {
		if (result.hasErrors()) {
			return VIEWS_USUARIOREGISTRADO_CREATE_OR_UPDATE_FORM;
		} else {
			usuarioRegistrado.setId(usuarioRegistradoID);
			//ERROR DESCONOCIDO producido en el metodo save del Servicio
			usuarioRegistradoService.saveUsuarioRegistrado(usuarioRegistrado);
			return "redirect:/usuariosRegistrados/" + usuarioRegistrado.getId();
		}
	}
	...
	public ModelAndView showUsuarioRegistrado(@PathVariable("usuarioRegistradoID") int usuarioRegistradoID) {
		ModelAndView mav = new ModelAndView(VIEWS_USUARIOREGISTRADO_SHOW);
		mav.addObject(usuarioRegistradoService.findUsuarioRegistradoById(usuarioRegistradoID));
		return mav;
	}
